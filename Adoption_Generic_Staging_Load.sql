...
-- For all Planning Choice records with an 'A' in NEW_ADD_DELETE add all fields regardless
-- of modified date.  This is to mitigate the situation where users add a row with information,
-- then adopt it at a future date. 
--
-- Delete all records besides Name and Planning Choice for records with an 'A' in NEW_ADD_DELETE.  
DELETE FROM mat_delta_sf_staging a
where a.magtemplates_id || a.row_id in 
(WITH MAGTEMP_ROWID_W_PC_ADD AS -- MAGTEMPATES_ID/ROW_ID with a Planning Choice record with NEW_ADD_DELETE = 'A'.
(
    SELECT magtemplates_id || row_id magtemp_row_combo 
    FROM mat_delta_sf_staging  WHERE sf_fieldname = 'Planning_Choice__c' 
    AND fieldvalue IS NOT NULL
    AND NEW_ADD_DELETE = 'A'
    order by magtemplates_id, row_id
)
select magtemp_row_combo from MAGTEMP_ROWID_W_PC_ADD)
and a.sf_fieldname <> 'Planning_Choice__c' 
and a.sf_fieldname <> 'Name';
COMMIT;

-- Add all fields regardless of modified date.
INSERT /*+ append  */ INTO MAT_DELTA_SF_STAGING
(
  MAG_ID,  MAG_NAME,  MAGTEMPLATES_ID,  MAGTEMPLATES_NAME,
  ROW_ID,  BRAND_ID,  BRAND_NAME,  SEASONS_ID,
...
INNER JOIN MDIS_MAGTEMPLATES C ON C.ID = A.MAGTEMPLATES_ID
where a.magtemplates_id || a.row_id in 
(SELECT d.magtemplates_id || d.row_id magtemp_row_combo 
    FROM mat_delta_sf_staging d  WHERE d.sf_fieldname = 'Planning_Choice__c' 
    AND d.fieldvalue IS NOT NULL
    AND d.NEW_ADD_DELETE = 'A')
and a.fieldvalue is not null
and b.salesforce_name <> 'Planning_Choice__c';
COMMIT;
...
--Step 15.4 : Add a NEW_ADD_DELETE record to every recordset.
INSERT /*+ append  */ INTO MAT_DELTA_SF_STAGING (
  MAG_ID,  MAG_NAME,  MAGTEMPLATES_ID,  MAGTEMPLATES_NAME,
  ROW_ID,  BRAND_ID,  BRAND_NAME,  SEASONS_ID,
  SEASON_YEAR,  SEASON_NAME, SEASON_SHORTNAME, SF_FIELDNAME,FIELDVALUE,SORT_ORDER,SEGMENT_COUNT,CREATE_DATE,MODIFIED_DATE,
...
SELECT MAG_ID, MAG_NAME, MAGTEMPLATES_ID, MAGTEMPLATES_NAME, 
ROW_ID, BRAND_ID, BRAND_NAME, SEASONS_ID, 
SEASON_YEAR, SEASON_NAME, SEASON_SHORTNAME, 'NEW_ADD_DELETE__c' AS SF_FIELDNAME, NEW_ADD_DELETE AS FIELDVALUE, 988 AS SORT_ORDER, 
SEGMENT_COUNT, CREATE_DATE, MODIFIED_DATE,'N' 
FROM MAT_DELTA_SF_STAGING WHERE SF_FIELDNAME = 'Planning_Choice__c';
COMMIT;

-----------------------------------------------------------
-- Step 16 : Trigger Fields - 09/27/2019 - We will pull regardless of date criteria for all planning choices in the extract file
