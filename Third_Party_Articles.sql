...
(
  ...
  VSS_PLAN_ORG,MASTERSTYLE,LINING_LEVEL,TICKET_PRICE,TREATMENT,COVERAGE,STRUCTURE,GRAPHIC,NAME
)
WITH ARTICLES AS 
(
    SELECT /*+MATERIALIZE */ MATNR FROM MPA_MASTER.SAP_MARA MARA 
    INNER JOIN MPA_MASTER.SAP_AUSP AUSP ON AUSP.OBJEK = MARA.MATNR AND AUSP.ATINN = 'MARKETPLACE'
...
SELECT LTRIM(MARA.MATNR,'0') ARTICLE__c, TO_CHAR(SUBRAND.ZZ_SUBRAND1) SUBRAND__c, TO_CHAR(SUBRAND.DESCR) SUBRAND_DESCR__c, 
       ...
       TICKET.ATTRIBUTE_VALUE TICKET_Price__c,
...
INNER JOIN MPA_MASTER.SAP_MARA MARA ON ART.MATNR = MARA.MATNR
       ...
       LEFT JOIN   ATTRIBUTES COVERAGE ON COVERAGE.OBJEK = MARA.MATNR  AND COVERAGE.ATTRIBUTE_NAME = 'COVERAGE'
       LEFT JOIN   ATTRIBUTES STRUCTURE ON STRUCTURE.OBJEK = MARA.MATNR  AND STRUCTURE.ATTRIBUTE_NAME = 'STRUCTURE'
       LEFT JOIN   ATTRIBUTES GRAPHIC ON GRAPHIC.OBJEK = MARA.MATNR  AND GRAPHIC.ATTRIBUTE_NAME = 'GRAPHIC'              
